...
import { ExternalServiceError, UnauthorizedError } from '../../../src/core/exceptions';
import { JwksTokenVerifier } from '../../../src/integrations/auth/jwks-token-verifier';
...
const verifyToken = (token: string, opts: typeof OPTS) => new JwksTokenVerifier(opts).verify(token);
...
describe('JwksTokenVerifier', () => {
  ...
  it('throws UnauthorizedError (401) on an invalid token', async () => {
    await expect(verifyToken('a.b', OPTS)).rejects.toBeInstanceOf(UnauthorizedError);
  });

  it('throws UnauthorizedError on an undecodable token', async () => {
    await expect(verifyToken('%%%.%%%.%%%', OPTS)).rejects.toThrow('Invalid token format');
  });

  it('throws ExternalServiceError (502) when the JWKS endpoint fails', async () => {
    const token = buildFakeToken(VALID_PAYLOAD);
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue({ ok: false, status: 500 }));

    await expect(verifyToken(token, { ...OPTS, jwksUrl: 'https://down.example.com/jwks' }))
      .rejects.toBeInstanceOf(ExternalServiceError);
  });

  it('throws ExternalServiceError when the JWKS endpoint is unreachable', async () => {
    const token = buildFakeToken(VALID_PAYLOAD);
    vi.stubGlobal('fetch', vi.fn().mockRejectedValue(new TypeError('network')));

    await expect(verifyToken(token, { ...OPTS, jwksUrl: 'https://unreachable.example.com/jwks' }))
      .rejects.toThrow('JWKS request failed');
  });
});
